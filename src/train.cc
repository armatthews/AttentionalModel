...
int main(int argc, char** argv) {
  ...
  if (!vm.count("model")) {
    ...
    if (!vm.count("sparsemax")) {
      attention_model = new StandardAttentionModel(dynet_model, 2 * embedding_dim + annotation_dim, output_state_dim, alignment_hidden_dim);
    }
    else {
      attention_model = new SparsemaxAttentionModel(dynet_model, 2 * embedding_dim + annotation_dim, output_state_dim, alignment_hidden_dim);
    }
