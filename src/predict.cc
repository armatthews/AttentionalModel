...
int main(int argc, char** argv) {
  ...
  const string input_source_pos = vm["input_source_pos"].as<string>();
  ...
  vector<InputSentence*> source_pos_tags = pos_reader->Read(input_source_pos);
  for (unsigned sentence_number = 0; sentence_number < source_sentences.size(); ++sentence_number) {
    ...
    KBestList<shared_ptr<OutputSentence>> kbest = translator.Translate(mfis, kbest_size, beam_size, max_length);
